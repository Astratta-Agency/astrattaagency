...
**Artículo nuevo → newsletter (automático):** `.github/workflows/newsletter-auto.yml` corre cuando Vercel termina un deploy de producción (y cada 6 h de respaldo). `scripts/newsletter/send-post.mjs --mode auto` envía cada artículo de los últimos 7 días cuya página ya responde 200 en producción y que no esté en `newsletter_sends`; la Edge Function `send-post-newsletter` lo manda con el artículo completo. Un artículo nunca sale dos veces. Pruebas y reenvíos manuales: workflow "Newsletter — enviar artículo" (modo `test` o `send`). Vista previa local: `node scripts/newsletter/send-post.mjs --mode preview`. Para que un artículo **no** se envíe, inserta su slug en `newsletter_sends` antes de publicarlo.
