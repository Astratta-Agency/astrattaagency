...
/**
 * Plantilla de las páginas legales (privacidad, términos): texto largo para
 * leer, no para escanear. Sin animaciones de scroll por sección — solo el
 * hero entra, el resto aparece ya en su sitio.
 */
export function LegalPage({
  copy: t,
  path,
}: {
  copy: Dictionary['privacy'] | Dictionary['terms']
  /** Ruta canónica en inglés; también es la clave de STATIC_SEO. */
  path: string
}) {
  return (
    <>
          ...
          <div className="flex max-w-2xl flex-col gap-12">
            {t.sections.map((section, index) => (
              <article key={section.heading}>
                ...
                {section.link && (
                  <Link to={section.link.to} className="mt-4 inline-block font-bold text-primary hover:text-ink">
                    {section.link.label} →
                  </Link>
                )}
              </article>
